...
import com.adblockerx.pro.services.FloatingBallService
...
fun HomeScreen(cfg: AdBlockConfig, onConfigChange: (AdBlockConfig) -> Unit) {
    ...
    val adsCount = remember { mutableStateOf(0L) }
    val trackersCount = remember { mutableStateOf(0L) }
    ...
    Column(
        ...
        Card(
            ...
            Column(modifier = Modifier.fillMaxWidth().padding(24.dp)) {
                ...
                Text("AdBlockerX Pro", style = MaterialTheme.typography.headlineSmall, fontWeight = FontWeight.Bold, color = MaterialTheme.colorScheme.onPrimaryContainer)
                Text("v${XposedLoader.VERSION}", style = MaterialTheme.typography.bodyMedium, color = MaterialTheme.colorScheme.onPrimaryContainer)
                Spacer(Modifier.height(8.dp))
                Text(
                    "已处理: ${adsCount.value + trackersCount.value} 次",
        ...
        Card(modifier = Modifier.fillMaxWidth().padding(bottom = 16.dp)) {
            Column(modifier = Modifier.padding(16.dp)) {
                ...
                Spacer(Modifier.height(8.dp))
                Row {
                    StatBox("已拦截广告", adsCount.value.toString(), modifier = Modifier.weight(1f))
                    StatBox("已拦截追踪", trackersCount.value.toString(), modifier = Modifier.weight(1f))
                }
            }
        }
