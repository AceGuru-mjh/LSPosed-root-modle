...
/**
 * 【实验性】本??VPN 拦截 Hook（Root 版独有）
 *
 * 功能??
 *  - Hook VpnService.Builder.establish() 拦截 APP 自身建立??VPN（避免与本模块冲突）
 *  - Hook ConnectivityManager（可选，记录网络切换??
 *
 * 设计思路??
 *  - 真正的本??VPN 拦截需要建??VpnService 进程，超??Xposed 模块范围
 *  - ??Hook 仅作??防止 APP 建立 VPN 绕过本模块拦??的对??
...
object VpnBasedBlockHook {
     ...
     * 拦截 APP 自身建立 VPN（防止绕过本模块??DNS/网络拦截??
     */
