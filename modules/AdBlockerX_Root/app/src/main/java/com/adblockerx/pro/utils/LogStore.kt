 ...
 *
 *  - 通过 SharedPreferences 持久化最近 200 条日志 + 一个累计计数器
 *  - 写入端：各 Hook / 关键事件（FloatingBallService 启动、拦截命中、配置变更等）
 *  - 读取端：PanelActivity / HomeScreen 实时展示
 *
 *  LSPosed 兼容：使用 MODE_WORLD_READABLE 写入，LSPosed 拦截并放行。
 *  Hook 进程若无法 init，则 add() / incrementCounter() 静默忽略（避免崩溃）。
 */
data class LogEntry(
...
object LogStore {
    ...
    private const val PREFS_NAME = "adblockerx_pro_log_store"
