 ...
 *
 *  - 通过 SharedPreferences 持久化最�?200 条日�?+ 一个累计计数器
 *  - 写入端：�?Hook / 关键事件（FloatingBallService 启动、拦截命中、配置变更等�?
 *  - 读取端：PanelActivity / HomeScreen 实时展示
 *
 *  LSPosed 兼容：使�?MODE_WORLD_READABLE 写入，LSPosed 拦截并放行�?
 *  Hook 进程若无�?init，则 add() / incrementCounter() 静默忽略（避免崩溃）�?
 */
data class LogEntry(
