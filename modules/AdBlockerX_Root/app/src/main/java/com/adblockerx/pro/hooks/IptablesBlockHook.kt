...
/**
 * 【实验性】iptables 网络层拦??Hook（Root 版独有）
 *
 * 功能??
 *  - ??APP 启动时通过 Shizuku 解析广告域名得到 IP
 *  - 添加 iptables OUTPUT 链规则：iptables -A OUTPUT -d <ad_ip> -j DROP
 *  - APP 退出时清理规则（避免影响其??APP??
 *
 * 注意事项??
 *  - 需 Root ??Shizuku shell 授权
 *  - iptables 规则仅对当前用户进程生效，规则残留可能影响系??
 *  - 广告域名 IP 可能动态变化，需周期刷新（本实现仅在启动时刷新一次）
 *  - 默认关闭，谨慎使??
 *
 * §4.2 命令执行??Hook：通过 Hook Application.onCreate 触发 Shizuku 命令执行
 * （创??iptables ??+ 解析域名 + 添加 DROP 规则），避免空壳??
 */
object IptablesBlockHook {
    ...
    fun apply(lpparam: XC_LoadPackage.LoadPackageParam, cfg: AdBlockConfig) {
        ...
        // §4.2 命令执行??Hook：Hook Application.onCreate 触发 iptables 规则添加
            ...
            object : XC_MethodHook() {
                override fun afterHookedMethod(p: MethodHookParam) {
                    ...
                    if (!ShizukuHelper.isShizukuAvailable()) {
                        LogX.w("Shizuku 不可用，跳过 iptables 拦截")
                        return
                    }
                    ensureChain()
                    addRules(ctx)
        ...
        LogX.hookSuccess("Application", "onCreate->Iptables")
    }

    /** 创建自定义链（避免污??OUTPUT 主链??*/
    private fun ensureChain() {
        // execShellSilent 内部已有 try-catch，失败返??false
        // 创建链（已存在会失败，忽略）
        ...
        LogX.d("iptables ??$CHAIN_TAG 已就??)
    }

    /** 解析广告域名??IP 并添??DROP 规则 */
    private fun addRules(@Suppress("UNUSED_PARAMETER") ctx: Context) {
        ...
        for (host in hosts.take(50)) {
            // 通过 Shizuku 解析域名；execShell 内部已有 try-catch，失败返??null
        ...
        LogX.i("iptables 规则添加完成??success ??DROP 规则")
    }
