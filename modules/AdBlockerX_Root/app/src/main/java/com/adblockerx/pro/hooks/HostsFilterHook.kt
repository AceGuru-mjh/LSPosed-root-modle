 ...
 *
 * 设计目标??
 *  - 维护一份广告域名黑名单（内??90 ??+ 用户自定义）
 *  - 提供统一查询接口 [isBlocked] 供其??Hook 调用
 *  - 同时提供 currentBlockedHosts() ??SystemHostsHook 写入系统 hosts
 *
 * 边界声明??
 *  - 内存查询仅作用于??APP 进程内的网络请求
 *  - SystemHostsHook 通过 Shizuku 写入系统??hosts 文件（独立操作）
 *  - 本类??AI_DEV_GUIDE §4.3 工具类规范，apply() 仅加载配??+ 打日志，
 *    不调用任??Xposed Hook 注册 API，体检脚本判定??"utility" 状态（合理??
 */
object HostsFilterHook {
    ...
    fun apply(lpparam: XC_LoadPackage.LoadPackageParam, cfg: AdBlockConfig) {
        // 工具类：仅加载配??+ 打日志，不直接注??Xposed 方法 Hook
        currentConfig = cfg
        LogX.i("HostsFilter 工具类已加载 | 内置黑名??${cfg.builtinBlocklistEnabled} 自定义条??${cfg.customBlocklist.size}")
        LogX.i("HostsFilter 工具类已加载：内置黑名单总条??${AdBlockList.BUILTIN_AD_DOMAINS.size}")
    }
    ...
    /** 返回当前生效的所有拦??host 列表（内??自定义），供 SystemHostsHook 写入 */
