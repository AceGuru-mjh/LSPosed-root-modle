 ...
 *
 * 包含 NoRoot 版全部应用层开??+ Root 版独有的系统级开关：
 *  - systemHostsEnabled: 通过 Shizuku 修改系统??hosts（Magisk overlay??
 *  - privateDnsEnabled: 通过 Shizuku 设置系统级广告过??Private DNS
 *  - dnsResolverHookEnabled: Hook 系统 DNS 解析返回 127.0.0.1
 *  - shizukuBridgeEnabled: Shizuku 桥接（刷??DNS 缓存等）
 *
 * 实验性（Root 版独有）??
...
data class AdBlockConfig(
    ...
    // ===== Root 版独有：系统级开??=====
    /** 系统??hosts 文件修改（Shizuku/Root，Magisk overlay??*/
    var systemHostsEnabled: Boolean = false,
    /** 系统??Private DNS 设置（Shizuku/Root??*/
    ...
    /** Shizuku 桥接（刷??DNS 缓存等） */
    var shizukuBridgeEnabled: Boolean = true,

    // ===== Root 实验??=====
    /** iptables OUTPUT ??DROP 广告 IP（Shizuku/Root??*/
