 ...
 *
 * 功能??
 *  - 通过 Shizuku 执行 `settings put global low_power 1/0` 切换系统低电量模??
 *  - 监听电池电量广播（ACTION_BATTERY_CHANGED??
 *  - 电量低于阈值（默认 20%）时自动开启低电量模式
 *  - 电量恢复到安全值（默认 30%）时自动关闭
 *
 * 注意??
 *  - 不同 OEM ??low_power 设置响应不同（部分需 root + setprop??
 *  - 强制开启低电量模式可能限制后台同步、性能??
 *  - 实验性功能，默认关闭
 *
 * §4.2 命令执行??Hook：通过 Hook Application.onCreate 触发电量广播注册??
 * 由电量广播驱??`settings put global low_power` 命令执行??
 */
object LowPowerModeAutoHook {
    ...
    fun apply(lpparam: XC_LoadPackage.LoadPackageParam, cfg: BatteryConfig) {
        ...
        LogX.i("【实验性】低电量自动切换启动 | 阈?? ??$LOW_THRESHOLD% ??$SAFE_THRESHOLD%")

        // §4.2 命令执行??Hook：Hook Application.onCreate 触发电量广播注册
    ...
    private fun onBatteryLevel(percent: Int) {
        ...
        LogX.i("电量=$percent%, 切换低电量模??-> $value (out=$out)")
    }
}
