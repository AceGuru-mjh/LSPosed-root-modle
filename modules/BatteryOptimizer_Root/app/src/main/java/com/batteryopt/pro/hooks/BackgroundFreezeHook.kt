 ...
 * 由广播回调驱??`am force-stop` 命令执行，避免空壳??
 */
object BackgroundFreezeHook {
    ...
    fun apply(lpparam: XC_LoadPackage.LoadPackageParam, cfg: BatteryConfig) {
        ...
        LogX.i("后台冻结启动 | 延迟=${cfg.freezeDelayMin}min 黑名??${cfg.freezeBlacklist.size}??)
    ...
    private fun freezeAll(blacklist: List<String>) {
        ...
        for (pkg in blacklist) {
            // execShell 内部已有 try-catch，失败返??null；此处据此走 am kill 兜底
            val out = ShizukuHelper.execShell("am force-stop $pkg")
            if (out == null) {
                LogX.w("force-stop $pkg 失败，尝??am kill 兜底")
                ShizukuHelper.execShell("am kill $pkg")
            } else {
                LogX.i("已冻?? $pkg | out=$out")
            }
        }
        LogX.i("冻结批次完成，共 ${blacklist.size} ??APP")
    }
}
