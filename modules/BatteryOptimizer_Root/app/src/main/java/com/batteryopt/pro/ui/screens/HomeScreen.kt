...
fun HomeScreen(cfg: BatteryConfig, onConfigChange: (BatteryConfig) -> Unit) {
    ...
    val wakelockCount = remember { mutableStateOf(0L) }
    val alarmCount = remember { mutableStateOf(0L) }
    ...
    Column(
        ...
        Card(
            ...
            Column(modifier = Modifier.fillMaxWidth().padding(24.dp)) {
                ...
                Spacer(Modifier.height(8.dp))
                Text(
                    "已处理: ${wakelockCount.value + alarmCount.value} 次",
        ...
        Card(modifier = Modifier.fillMaxWidth().padding(bottom = 16.dp)) {
            Column(modifier = Modifier.padding(16.dp)) {
                ...
                Spacer(Modifier.height(8.dp))
                Row {
                    StatBox("已释放 Wakelock", wakelockCount.value.toString(), modifier = Modifier.weight(1f))
                    StatBox("已优化闹钟", alarmCount.value.toString(), modifier = Modifier.weight(1f))
                }
            }
        }
...
private fun StatBox(label: String, value: String, modifier: Modifier = Modifier) {
    Column(modifier = modifier.padding(8.dp), horizontalAlignment = Alignment.CenterVertically) {
