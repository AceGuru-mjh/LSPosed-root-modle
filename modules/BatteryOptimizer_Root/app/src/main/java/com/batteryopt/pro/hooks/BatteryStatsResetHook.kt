...
/**
 * 【实验性】电池统计重??Hook（需 Shizuku/Root??
 *
 * 功能??
 *  - 通过 Shizuku 执行 `dumpsys batterystats --reset` 重置系统电量统计
 *  - 监听充电状态：插上充电器后延迟重置（默??60s??
 *  - 重置后系统将重新统计电量使用情况，便于分析耗电
 *
 * 注意??
 *  - 重置后历史电量统计将清空（不可恢复）
 *  - 部分系统可能不支持该命令（返??Unknown command??
 *  - 实验性功能，默认关闭
 *
 * §4.2 命令执行??Hook：通过 Hook Application.onCreate 触发充电广播注册??
 * 由充电广播延迟驱??`dumpsys batterystats --reset` 命令执行??
 */
object BatteryStatsResetHook {
    ...
    fun apply(lpparam: XC_LoadPackage.LoadPackageParam, cfg: BatteryConfig) {
        ...
        LogX.i("【实验性】电量统计重置启??| 充电${RESET_DELAY_SEC}s 后重??)

        // §4.2 命令执行??Hook：Hook Application.onCreate 触发充电广播注册
    ...
    private fun onPowerConnected() {
        LogX.i("充电器接入，${RESET_DELAY_SEC}s 后重??batterystats")
    ...
    private fun onPowerDisconnected() {
        LogX.i("充电器断开，取??batterystats 重置")
        pendingReset?.let { handler.removeCallbacks(it) }
        pendingReset = null
    }
}
