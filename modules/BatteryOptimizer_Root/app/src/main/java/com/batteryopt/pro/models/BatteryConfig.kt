 ...
 *
 * 包含两套配置??
 *  1. 应用??Hook 开关（??NoRoot 版同步）
 *  2. 系统??Hook 开关（??Root 版，依赖 Shizuku??
 *  3. 实验性功能开??
 */
data class BatteryConfig(
    ...
    /** 屏幕亮起时使用的 governor（默??interactive??*/
    var cpuGovernorActive: String = "interactive",
    /** 屏幕关闭时使用的 governor（默??powersave??*/
    ...
    /** 屏幕关闭时是否启??zram（true=启用压缩冷数据，false=完全禁用??*/
