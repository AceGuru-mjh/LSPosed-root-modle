 ...
 *
 * 通过 Shizuku 执行 tinymix 获取/设置 ALSA 混音器控件。
 * 用于硬件级音量、增益、路由控制。
 * 硬性限制：需 Shizuku root 级授权 + 设备支持 tinymix 二进制
 */
object TinymixAudioHook {
    ...
    private fun execute(cfg: AudioConfig) {
        ...
        // 1. 探测 tinymix 二进制路径
        val tinymixPath = findTinymixPath(sh)
        if (tinymixPath == null) {
            LogX.w("[root] 设备不支持 tinymix，跳过混音器控制")
            return
        }
        ...
        if (mixerList != null) {
            ...
            // 4. 额外探测每路扬声器
            probeExtraControls(sh, tinymixPath, mixerList)
        }
    }
    ...
    private fun probeExtraControls(sh: ShizukuHelper, tinymixPath: String, mixerList: String) {
        ...
        for (control in extraControls) {
            if (mixerList.contains(control)) {
                ...
                if (valOut != null) {
                    LogX.d("[root] tinymix $control 当前值: ${valOut.trim()}")
                }
            }
        }
    }
