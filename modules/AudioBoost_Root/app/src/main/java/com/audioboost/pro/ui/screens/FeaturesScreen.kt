...
fun FeaturesScreen(cfg: AudioConfig, onConfigChange: (AudioConfig) -> Unit) {
    ...
    Column(
        ...
        FeatureCard(
            "音质增强", "44100Hz→48000Hz 采样率提升 + 16→24bit 位深提升",
        ...
        FeatureCard(
            "系统音量突破上限", "Shizuku media volume --set 突破系统最大音量限制",
        ...
        FeatureCard(
            "AudioPolicy Hack", "Magisk overlay 持久化 audio_policy_configuration.xml（+6dB 硬件增益）",
        ...
        FeatureCard(
            "tinymix 混音器探测", "Shizuku 执行 tinymix 获取/设置 ALSA 混音器控件（硬件级增益）",
