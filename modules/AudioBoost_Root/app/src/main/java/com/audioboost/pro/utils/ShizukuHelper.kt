 ...
 *     - media volume --set 设置系统音量突破上限
 *     - cmd media_audio 执行系统音频命令
 *     - ??/sys/class/audio/pcm 等系统节??
 *     - 修改 AudioPolicy 配置
 *
 * 硬性限制：
...
object ShizukuHelper {
    ...
    /** 仅执行不关心输出，返回是否执行成功（未抛异常??Shizuku 可用??*/
    ...
    /**
     * 通过 Shizuku 设置系统音量（突破上限）
     * ??cmd media_audio 调用 audio service 设置音量到指定刻??
     */
    fun setSystemVolume(streamType: Int, volume: Int): Boolean {
        return execShellSilent("media volume --show --stream $streamType --set $volume")
    }
    ...
    /**
     * 执行 cmd media_audio 命令
     * ?? cmd media_audio list
     */
    fun execMediaAudio(subCmd: String): String? {
        return execShell("cmd media_audio $subCmd")
    }
