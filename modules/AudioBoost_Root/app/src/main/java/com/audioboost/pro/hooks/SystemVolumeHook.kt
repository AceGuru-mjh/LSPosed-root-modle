 ...
 *  - Hook AudioManager.setStreamVolume 拦截应用层调用，??systemVolumeMaxBoost 放大后通过 Shizuku 写入
 *  - 监听 SCREEN_ON 广播定期重置（避免被系统回滚??
 *
 * 硬性限制：
 *  - 必须先检??ShizukuHelper.isShizukuAvailable()
 *  - 系统级音量修改可能被系统回滚，需??SCREEN_ON 时重新应??
...
object SystemVolumeHook {
    ...
    fun apply(lpparam: XC_LoadPackage.LoadPackageParam, cfg: AudioConfig) {
        ...
        LogX.i("系统级音量突破启??boost=${cfg.systemVolumeMaxBoost}%")
        ...
        // Hook AudioManager.setStreamVolume 拦截应用层调??
    ...
    private fun applySystemVolume(cfg: AudioConfig) {
        ...
        try {
            ...
            LogX.i("系统级音?? max=$maxIdx -> 目标=$target")
            // 通过 media volume --set 设置（部??Android 12+ 支持??
            val ok = ShizukuHelper.setSystemVolume(3, target)
            // 备选方?? 通过 audio service set-stream-volume
            if (!ok) {
