 ...
 *  - cmd media_audio 命令??Android 版本差异较大
 *  - 强制路由可能与系统状态冲突，谨慎使用
 */
object ShizukuAudioBridgeHook {
    ...
    fun apply(lpparam: XC_LoadPackage.LoadPackageParam, cfg: AudioConfig) {
        ...
        // Hook Application.onCreate 触发音频桥接初始??
        try {
        ...
        // Hook AudioManager.setMode 拦截应用层切换，确保扬声器始终启??
        hookAudioManagerSetMode(lpparam, cfg)
    }

    /** 通过 Shizuku 执行 cmd media_audio 初始化音频桥??*/
    private fun initAudioBridge(cfg: AudioConfig) {
        ...
        try {
            ...
            LogX.d("设置媒体音量??$targetVol: $ok2")
        } catch (e: Throwable) {
            LogX.e("Audio Bridge 初始化异??, e)
        }
    }

    /** Hook AudioManager.setMode 拦截应用层切??*/
