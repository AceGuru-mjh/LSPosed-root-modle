...
/**
 * 鍧囪　鍣℉ook锛堜粎搴旂敤灞傦紝涓?NoRoot 鐗堢浉鍚岋級
 *
 * 鎷︽埅璺緞锛?
 *  1. Equalizer.setBandLevel(short, short)
 *  2. Equalizer.getBandLevel(short)
 *  3. Equalizer.setBandLevels(short[], short[])
 *  4. Equalizer.getNumberOfBands()
 */
object EqualizerHook {

    fun apply(lpparam: XC_LoadPackage.LoadPackageParam, cfg: AudioConfig) {
        if (!cfg.equalizerEnabled) return
        LogX.i("鍧囪　鍣ㄥ惎鍔?bands=${cfg.eqBands}")
