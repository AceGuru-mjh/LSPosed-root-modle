...
/**
 * 游戏模式激??Hook（实验性，系统级，需 Shizuku adb 级授权）
 *
 * 功能??
 *  - 通过 Shizuku 执行 `cmd game_mode <mode>` 设置游戏模式
 *  - 通过 Shizuku 执行 `settings put global game_mode 2` 强制游戏模式
 *  - Hook GameManager / GameModeManager 让游戏自身感知已激活游戏模??
 *
 * 硬性限制：
 *  - `cmd game_mode` 命令仅在 Android 12+ 可用
 *  - 不同厂商??game_mode 的支持差异较??
 *  - settings put global game_mode 为非持久化修改，重启后失??
 *
...
object GameModeActivationHook {
    ...
    private fun activateGameModeViaShizuku() {
        ...
        LogX.i("Shizuku 游戏模式已激??)
    }

    /** Hook GameManager 让应用感知已激活游戏模??*/
