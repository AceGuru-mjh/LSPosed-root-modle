...
object ShizukuBridgeHook {
    ...
    fun apply(lpparam: XC_LoadPackage.LoadPackageParam, cfg: GameConfig) {
        ...
        // 同时 Hook Application.onCreate 以确??ShizukuHelper 已初始化
        hookAppLifecycle(lpparam)
    }

    /** 通过 Shizuku setprop 修改刷新率属??*/
    private fun applyRefreshRateProps(targetFps: Int) {
        if (!ShizukuHelper.isShizukuAvailable()) {
            LogX.w("Shizuku 不可用，跳过 setprop 系统属性修??)
            return
        }
            ...
            XposedHelpers.findAndHookMethod(
                ...
                object : XC_MethodHook() {
                    override fun afterHookedMethod(p: MethodHookParam) {
                        // Shizuku 状态在 onCreate 后再次确??
                        if (ShizukuHelper.isShizukuAvailable()) {
                            LogX.d("Shizuku 状态确?? 可用")
                        }
                    }
                })
