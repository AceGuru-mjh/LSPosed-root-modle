 ...
 *  - Shizuku pm clear 清理 APP 追踪数据（缓??数据库）
 *  - Hook Application.onCreate ??APP 启动后清??adid 缓存
 *
 * 硬性限制：
 *  - 必须先检??ShizukuHelper.isShizukuAvailable()
 *  - pm clear 会清??APP 所有数据（包括登录态），谨慎使??
...
object ShizukuBridgeHook {
    ...
    private fun clearAppTrackingData(pkg: String) {
        ...
        if (result != null) {
            LogX.w("Shizuku 已执??pm clear: $pkg（注意：登录态会丢失??)
        }
    }
