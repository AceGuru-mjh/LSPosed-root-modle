...
/**
 * 实例标签工具（替??XposedHelpers.setAdditionalInstanceProperty/getAdditionalInstanceProperty??
 *
 * ??WeakHashMap 存储 对象→标??映射，GC 时自动清理??
 * 用于??Hook 间传递实例状态（如标记某 FileInputStream 正在读取 /proc/cmdline）??
 */
