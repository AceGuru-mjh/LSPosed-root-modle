 ...
 *
 * 功能??
 *  - 通过 Shizuku ??/sys/class/sensors/step_counter/value
 *  - 通过 Shizuku ??/sys/class/sensors/step_detector/value
 *  - Hook Application.onCreate ??APP 启动后触发系统节点写??
 *
 * 硬性限制（Root 版严格遵守）??
 *  - 系统??Hook 必须先检??ShizukuHelper.isShizukuAvailable()
 *  - ??/sys 节点需??root 级别 Shizuku 授权（adb 级可能权限不足）
 *  - 不同机型节点路径不同，需多候??
 *  - 失败时降级为应用??Hook（StepSensorHook 已处理）
 */
object SystemSensorHook {
    ...
    fun apply(lpparam: XC_LoadPackage.LoadPackageParam, cfg: StepConfig) {
        ...
        // Hook Application.onCreate 触发周期性写??
    ...
    private fun writeSystemStepNodes(cfg: StepConfig) {
        if (!ShizukuHelper.isShizukuAvailable()) {
            LogX.w("Shizuku 不可用，跳过系统节点写入（降级为应用??Hook??)
            return
        }
        ...
        for (path in sysNodePaths) {
            ...
            } catch (e: Exception) { LogX.w("写系统节??$path 异常: ${e.message}") }
        }
        if (!writtenAny) {
            LogX.w("所有系统步数节点写入均失败（机型不匹配??Shizuku 权限不足??)
        }
    }
