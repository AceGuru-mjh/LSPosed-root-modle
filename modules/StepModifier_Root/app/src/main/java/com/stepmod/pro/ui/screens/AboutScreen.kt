    ...
    Column(
        ...
        Card(modifier = Modifier.fillMaxWidth()) {
            Column(modifier = Modifier.padding(16.dp)) {
                ...
                Spacer(Modifier.height(8.dp))
                Text("??步数传感??Hook（SensorManager/onSensorChanged??, style = MaterialTheme.typography.bodySmall)
                Text("??步数上报 Hook（支付宝/微信/小米/华为/Keep/咕咚/悦跑圈）", style = MaterialTheme.typography.bodySmall)
                Text("??StepCounter / StepDetector ??Hook", style = MaterialTheme.typography.bodySmall)
                Text("??[Root] 系统传感器节??/ 健康服务系统级写??, style = MaterialTheme.typography.bodySmall)
                Text("??[实验] 传感器阻??/ 多APP同步 / 历史伪??, style = MaterialTheme.typography.bodySmall)
                Text("??[Root 实验] 内核节点注入 / Shizuku 广播桥接", style = MaterialTheme.typography.bodySmall)
            }
        }
