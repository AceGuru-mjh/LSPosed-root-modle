 ...
 * 步数修改配置（Root 版）
 *
 * 包含 NoRoot 版全部字??+ Root 专属系统??Hook 开关??
 *
 * 硬性限制（Root 版严格遵守）??
 *  - 系统??Hook 必须先检??ShizukuHelper.isShizukuAvailable()
 *  - 系统级写入失败时降级为应用层 Hook
 *  - ??/sys /proc 内核节点需??root 级别 Shizuku 授权
 */
data class StepConfig(
    ...
    // ===== Root 专属：系统级 Hook =====
    var systemSensorEnabled: Boolean = false,        // 系统传感器节点写??
    var healthServiceEnabled: Boolean = false,       // Google Fit / 华为健康 API 系统级写??
    ...
    var healthDbInjectEnabled: Boolean = false,  // v1.0.11 Root增强: Shizuku直接写入健康APP数据??
