 ...
 *  - 命中关键词过滤时不调用原 notify，而是通过 Shizuku 执行 cmd notification post 重新发??
 *
 * 硬性限制：
 *  - 需 Shizuku adb 级授??
 *  - cmd notification post 需??--pkg / --tag / --id 等参数，构造复??
 *  - 实验性：可能??Android 版本差异导致 post 失败
 */
object ShizukuNotifyBridgeHook {
    ...
    private fun hookNotify(lpparam: XC_LoadPackage.LoadPackageParam, cfg: NotifyConfig) {
        ...
        // Hook notify(int, Notification) ??after 阶段尝试通过 Shizuku 同步刷新
        try {
        ...
        // Hook notifyAsUser（Android 8+?? ??after 阶段记录 cmd notification post 日志（仅诊断，不实际 post??
        try {
            XposedHelpers.findAndHookMethod(
                nmCls, "notifyAsUser",
                String::class.java,
                ...
                object : XC_MethodHook() {
                    override fun afterHookedMethod(p: MethodHookParam) {
                        try {
                            ...
                            // 仅诊断日志：实际 cmd notification post 需??XML 参数，这里不构??
        ...
        // Hook cancel(int) - ??after 阶段通过 Shizuku 兜底刷新（避免应??cancel 后系统仍残留??
        try {
            XposedHelpers.findAndHookMethod(
                nmCls, "cancel",
                Int::class.javaPrimitiveType,
