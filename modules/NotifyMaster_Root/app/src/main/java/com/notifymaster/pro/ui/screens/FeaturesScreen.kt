...
fun FeaturesScreen(cfg: NotifyConfig, onConfigChange: (NotifyConfig) -> Unit) {
    ...
    Column(
        ...
        FeatureCard(
            "全局通知过滤", "??APP 通知过滤（Hook Listener + cmd notification cancel??,
        ...
        FeatureCard(
            "全局通知队列管理", "cmd notification list/cancel/post ??APP 通知排序/延迟/合并",
        ...
        FeatureCard(
            "Sysfs 通知 LED", "通过 Shizuku ??/sys/class/leds 节点控制通知??+ 振动模式",
            cfg.sysfsLedEnabled,
