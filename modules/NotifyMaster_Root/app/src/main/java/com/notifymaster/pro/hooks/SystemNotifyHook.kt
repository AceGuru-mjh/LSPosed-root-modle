 ...
 *     - settings put secure show_notification_snooze（隐??snooze??
...
object SystemNotifyHook {
    ...
    fun apply(lpparam: XC_LoadPackage.LoadPackageParam, cfg: NotifyConfig) {
        ...
        // Hook Application.onCreate ??APP 启动后同步策??
        try {
        ...
        // 立即同步一次（??Hook 注册阶段，Shizuku 可能还没绑定，仅尝试??
    ...
    private fun applySystemNotifyPolicy(ctx: Context?, cfg: NotifyConfig) {
        ...
        // 3. 全局最低优先级（设??NotificationManagerService 配置??
        // Note: 系统层面无直??最低优先级"全局参数，这里通过 settings put secure 配置
        if (cfg.globalImportanceFloor > 0) {
            try {
                ...
                LogX.d("已设置全局最低通知优先??floor=${cfg.globalImportanceFloor}")
