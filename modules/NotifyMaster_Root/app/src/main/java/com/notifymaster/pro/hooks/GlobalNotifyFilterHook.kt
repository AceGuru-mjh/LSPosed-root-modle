 ...
 *  - 在回调中提取通知文本，命中关键词??cmd notification cancel <key>
 *
 * 硬性限制：
 *  - 需 LSPosed 加载??system_server 或对??NotificationListenerService 进程
 *  - cmd notification cancel 需??Shizuku adb 级授??
...
object GlobalNotifyFilterHook {
    ...
    private fun hookListenerForFilter(lpparam: XC_LoadPackage.LoadPackageParam, cfg: NotifyConfig) {
        val listenerCls = XposedHelpers.findClassIfExists(
            ...
            LogX.w("NotificationListenerService 类未找到，全局过滤不可??)
            return
        }
