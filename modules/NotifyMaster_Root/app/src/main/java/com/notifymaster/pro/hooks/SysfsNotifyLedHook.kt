 ...
 * 通过 Shizuku ??/sys/class/leds 节点控制通知指示灯和振动模式?? * 硬性限制：需 Shizuku root 级授?? */
object SysfsNotifyLedHook {
    ...
    private fun execute() {
        ...
        // 探测可用??LED 节点并点亮红??        for (path in ledPaths) {
            val result = sh.execShell("echo 255 > $path 2>/dev/null")
            if (result != null) {
                LogX.d("[root] LED 节点已写?? $path -> 255")
                break
            }
        }
        ...
        LogX.d("[root] 通知 LED trigger 模式已激??)
    }
}
