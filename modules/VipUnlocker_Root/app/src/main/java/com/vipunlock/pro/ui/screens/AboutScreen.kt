    ...
    Column(
        ...
        Card(modifier = Modifier.fillMaxWidth()) {
            Column(modifier = Modifier.padding(16.dp)) {
                ...
                Text("??[实验] 通用VIP/去广??绕过校验", style = MaterialTheme.typography.bodySmall)
                Text("??[Root] 系统属性伪装VIP/Google License授权", style = MaterialTheme.typography.bodySmall)
                Text("??[Root 实验] Shizuku权限桥接/hosts全局广告屏蔽", style = MaterialTheme.typography.bodySmall)
            }
        }
