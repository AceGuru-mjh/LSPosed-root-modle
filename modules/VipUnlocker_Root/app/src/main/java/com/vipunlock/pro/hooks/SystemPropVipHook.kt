...
/**
 * 系统属性伪??Hook（Root 专属，需 Shizuku adb 级授权）
 *
 * 目标：让 APP 通过 ro.product.model / ro.product.brand 等读取到高端机型标识??
 * 部分APP据此开??高端机型专享"??VIP 权益（如 B??4K/杜比、爱奇艺星钻）??
 *
 * 双通道实现??
 *  1. 应用??Hook android.os.SystemProperties.get 拦截读取
 *  2. 通过 Shizuku setprop 实际修改系统属性（影响 APP 所有进程，但重启后消失??
 *
 * 硬性限制：
 *  - 必须先检??ShizukuHelper.isShizukuAvailable()
 *  - ro.* 属性原生不可写，setprop 对部分只读属性无效（应用??Hook 兜底??
 *  - 持久化需写入 build.prop（需??root ??Shizuku），重启后消??
 */
object SystemPropVipHook {
    ...
    /** 应用??Hook SystemProperties.get 拦截读取 */
    private fun hookSystemProperties(lpparam: XC_LoadPackage.LoadPackageParam, cfg: VipConfig) {
        try {
            ...
            LogX.i("SystemProperties Hook完成: ${props.size}个属??)
        } catch (e: Exception) {
            LogX.hookFailed("SystemProperties", "get", e)
        }
    }

    /** Hook Build 静态字段（部分APP直接??Build.MODEL??*/
    ...
    private fun applySystemPropsViaShizuku(cfg: VipConfig) {
        ...
        LogX.i("Shizuku setprop 完成: $success/${props.size} 个属性设置成??)
    }
}
