 ...
 *
 * 目标：通过 Shizuku 修改 /system/etc/hosts ??/data/adb/hosts??
 * 全局屏蔽广告域名（影响所有APP，不仅当前APP）??
 *
 * 双通道实现??
 *  1. 应用??Hook InetAddress.getAllByName / Network.getAllByName 拦截广告域名解析
 *  2. Shizuku 追加规则??/system/etc/hosts（需 root 级，??/system 可写??Magisk overlay??
 *
 * 触发方式??
 *  - Hook Application.onCreate ??APP 启动后追??hosts 规则
 *
 * 硬性限制：
 *  - 必须先检??ShizukuHelper.isShizukuAvailable()
 *  - 修改 /system/etc/hosts 需??/system 可写??Magisk overlay，否则失??
...
object GlobalAdBlockHook {
    ...
    /** 应用??Hook InetAddress.getAllByName 拦截广告域名解析 */
    ...
    private fun isBlocked(host: String, blocked: HashSet<String>): Boolean {
        ...
        // 子域名匹配：ad.toutiao.com 子域名包??ad.toutiao.com 即屏??
        for (b in blocked) {
    ...
    private fun appendHostsRules(cfg: VipConfig) {
        ...
        // 追加??/system/etc/hosts（可能因只读失败，降级到 /data/adb/modules/vipunlocker/system/etc/hosts??
        val content = sb.toString()
        // 尝试方案1：直接追??/system/etc/hosts（root 级，需 /system 可写??
        val r1 = ShizukuHelper.execShell("echo '$content' >> /system/etc/hosts 2>/dev/null")
        if (r1 != null) {
            LogX.i("已追??${cfg.blockedAdDomains.size} 条规则到 /system/etc/hosts")
            return
        }
        ...
        LogX.i("已追??${cfg.blockedAdDomains.size} 条规则到 $magiskPath (Magisk overlay)")
    }

    fun release() {
        isApplied = false
        LogX.d("GlobalAdBlock 资源已释??)
    }
}
