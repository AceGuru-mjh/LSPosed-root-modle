 ...
 *
 * 分两层：
 *  1. applyForXxx: ??APP 专属候选类名表（酷??酷我/优酷/腾讯视频/喜马拉雅/今日头条/知乎/百度网盘/WPS/微信读书??
 *  2. applyForCommon: ??APP 通用方法??Hook（isVip / isPremium / getVipLevel 等通用命名??
 *
 * 硬性限制：
 *  - ??Hook 应用进程??VIP 状态查询方??
 *  - 服务端鉴权不绕过
...
object UniversalVipHook {
    ...
    fun applyForKugou(lpparam: XC_LoadPackage.LoadPackageParam, cfg: VipConfig) {
        ...
        try { LogStore.add("unlocked", "VIP解锁已启??) } catch (_: Exception) { }
    ...
    fun applyForCommon(lpparam: XC_LoadPackage.LoadPackageParam, cfg: VipConfig) {
        ...
        // Hook 通用枚举/常量类（部分APP??enum 表示VIP状态）
    ...
    /**
     * Hook 通用 VIP 枚举??
     * 候选：com.xxx.VipType / com.xxx.MemberType ??
    ...
    private fun hookCommonVipEnum(lpparam: XC_LoadPackage.LoadPackageParam) {
        ...
        for (clsName in enumCandidates) {
            val cls = XposedHelpers.findClassIfExists(clsName, lpparam.classLoader) ?: continue
            try {
                // Hook 构造或 ordinal 不通用，这??Hook 静态获取方??
