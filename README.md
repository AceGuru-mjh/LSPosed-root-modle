...
| # | 模块 | 功能 | 关键能力 |
|---|------|------|---------|
| 1 | **GameUnlockerPro** | 游戏性能 | 帧率解锁 / 温控绕过(sysfs) / GPU调频 / 内核调优 / 机型伪装 |
| 2 | **PrivacyGuard** | 隐私保护 | 设备ID伪造 / Magisk overlay 持久化 / SELinux chcon / /proc/cmdline挂载 |
| 3 | **AdBlockerX** | 广告拦截 | 系统hosts / PrivateDNS / DNS缓存刷新 / iptables / DoH |
| 4 | **BatteryOptimizer** | 省电优化 | Doze强制 / 应用冻结 / CPU governor / zram优化 / GPU降压 / appops |
| 5 | **StepModifier** | 步数修改 | 传感器HAL直写 / Google Fit/华为/小米 health DB注入 |
| 6 | **NotifyMaster** | 通知管理 | notification_policy.xml直写 / 监听器注入 / 全局通知队列 |
| 7 | **AudioBoost** | 音量增强 | tinymix硬件增益 / audio_effects.xml注入 / AudioPolicy修改 |
| 8 | **VipUnlocker** | VIP解锁 | Play Store DB修改 / Magisk overlay持久化 / 系统属性伪装 |
| 9 | **VideoSaver** | 视频下载 | screencap/screenrecord / 系统代理 / 媒体扫描触发 |
